...
async function connectToBridge(config: BridgeConfig): Promise<ImapFlow> {
  const password = await getBridgePassword(config.username);
  ...
  try {
    ...
    throw new Error(
      `Could not connect to Proton Mail Bridge at ${config.host}:${config.port}. ` +
        'Make sure Proton Mail Bridge is running and you are signed in.' +
